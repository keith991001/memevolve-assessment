...
MemEvolve 的核心问题不是"agent 要不要记忆"，而是：**记忆系统本身能不能像 agent policy 一样被自动设计和进化**。普通 memory 方法通常预先固定一种形态（例如 episodic trajectory、semantic insight 或技能库），再让 agent 往里写内容；MemEvolve 则把"如何编码、存储、检索、维护记忆"本身当成可搜索对象。

### 方法

论文先用 EvolveLab 把已有记忆系统统一进一个模块化设计空间。任何 memory provider 都被拆成四个组件：

| 模块 | 作用 |
|---|---|
| Encode | 把原始经验、轨迹或反馈转成可存储的记忆表示 |
| Store | 决定记忆如何持久化、索引和组织 |
| Retrieve | 根据当前任务上下文召回相关记忆 |
| Manage | 对记忆做维护、合并、剪枝、遗忘或更新 |

这四元组构成 memory architecture 的"基因型"，让架构层面的进化变得可操作。

MemEvolve 在这个设计空间上做双层优化：

1. **内环（经验进化）**：固定一组候选记忆架构，让 agent 带着每个架构跑一批任务，往记忆库里写入经验，同时收集任务成功率、token 成本和延迟。
2. **外环（架构进化）**：用 Pareto 排序在性能、成本、延迟之间做非支配筛选，保留较优架构作为父代；再让 LLM 回放轨迹、诊断结构性缺陷，并在 E/U/R/G 四个模块的约束内生成新的 memory provider。

两个环形成正反馈：更好的记忆架构让 agent 学得更快，更强的 agent 又会产生更高质量的轨迹，给外环提供更有信息量的适应度信号。

![MemEvolve 方法架构图](assets/memevolve_architecture.png)

### 主要结论

1. **性能**：在 GAIA、xBench-DS、WebWalkerQA、TaskCraft 等基准上，MemEvolve 相比无记忆或手写 memory baseline 有明显提升，而且成本没有同步显著上升。
2. **泛化性**：在 TaskCraft 上进化出的记忆系统，可以迁移到更难的 benchmark、不同底座模型和不同 agent 框架上。这个结果说明它学到的不是某个数据集的题目答案，而是更一般的记忆设计原则。
3. **进化出的设计规律**：从 AgentKB 到 Riva、Cerebra 的进化轨迹里，可以看到记忆系统逐渐变得更 agentic：编解码更依赖 agent 自主决策，记忆组织更层级化、多粒度化，后期还会从经验中蒸馏可复用工具并定期维护记忆库。

我自己的实验主要围绕这个主张展开：当 memory architecture 被固定为不同形态时，它到底在具体任务轨迹里带来什么收益、什么伤害，以及这些现象能否反过来解释 MemEvolve 为什么要做架构级 meta-evolution。
...
> **统计口径**：每组只有 20 条任务且只跑一次，本文不把准确率差异解释为统计显著结论，而主要关注 memory 注入如何改变具体任务轨迹、成本和失败机制。
...
### Case C（任务 9）：episodic / semantic 检索召回无关案例——北京地铁第二近站点

问"截至 2024 年北京地铁里哪两个独立站点轨道距离第二近"，golden=16 号线玉渊潭东门–木樨地。**只有 ExpeL 答错**（答 14 号线高家园–望京南 676 米），No-Memory / Lightweight / Voyager 都答对。

看轨迹就清楚了：ExpeL 在第 0 步注入的"相似成功案例"是一道**完全无关**的题——B 站「两面包夹芝士」梗视频里最后提到的明日方舟干员职业。更准确地说，这里直接出问题的是 ExpeL 的 episodic trajectory retrieval；由于 ExpeL 还会把成功轨迹蒸馏成 semantic insights，我把它归为 episodic / semantic 混合检索失效，而不是纯 semantic insight 错误。一道地铁距离题被塞进一个游戏梗解题流程，agent 顺着这条噪声跑偏，最终锁定了错误的线路。注入原文见附录 E。

这是 episodic / semantic memory 最典型的失效模式：**检索按"相似"召回，但"看起来相似"和"真正相关"是两回事**；一旦召回无关案例，它不只是没用，还会主动把 agent 带偏。这也是 ExpeL 三组里成本最高（§2，+45% token）的部分原因——它在噪声引导下做了更多无效检索。

### Case D（任务 17）：当前 memory 形态帮不上忙——历任校长姓氏统计

边界裁量加聚合统计的题（哪些前身机构的校长要算进去），golden=王。No-Memory、Lightweight、ExpeL 全错（都把存疑人选算了进去，答"陈王并列"），唯一答对的 Voyager 其实是零注入状态，纯属又采样了一次的运气。这里的瓶颈不是经验召回，而是边界规则、证据裁量和聚合验证；如果 memory 存的是明确裁量规则或统计模板，理论上可能有帮助，但本实验里的几种 memory 形态没有覆盖到这个层面。
...
写完上面的实验分析后，我补充调研了几篇"记忆系统自进化"方向的相关工作，并挑出三篇和 MemEvolve 对比。这里的目的不是把 §6 合并成一套更散的改进清单，而是用同类工作说明：我在 §5/§6 里指出的问题，在领域内分别对应哪些已有解法或可借鉴方向。
...
| 评测 | GAIA / xBench / WebWalkerQA / TaskCraft | 长对话/QA 基准 | WebArena / Mind2Web / SWE-Bench-Verified | 多 agent 基准，对 ExpeL/AWM 等 baseline 有明显提升 |
...
4. **注入时机可学习**（针对成本失控）。Lightweight 每 3 步无条件注入是写死的，MemGen 的 memory trigger 证明"该不该注入"本身可学。不训练参数的轻量做法：注入前加一次小模型判定，触发率纳入 fitness；同时把单任务 token 上限做成 Manage 模块的熔断算子——AFlow 这类工作也说明，推理成本可以作为显式优化目标，而不是事后统计项。
...
6. **设计空间本身可以再进化**（方法论展望，呼应 §6）。MemGen 说明 latent memory 也是一条可行方向，Gödel Agent（ACL 2025）一类工作则提醒我们：人工限定的设计空间存在表达力天花板。E/U/R/G 接口里没有"注入时机"和"记忆载体形态"这两个维度，它们未来也应进入可进化范围。
...
这次实验没有得到"memory 一定提升准确率"这种简单结论。相反，在前 20 条 xBench-DeepSearch 上，无记忆基线已经很强，三种 memory setting 都没有带来稳定准确率收益，反而普遍增加 token 和调用成本。更有价值的发现是机制层面的：memory 的收益来自任务状态保持、搜索策略迁移和工具使用经验；memory 的伤害则来自错误写入、无关检索、缺少事实验证和缺少成本止损。

因此，我对 MemEvolve 的理解是：它的贡献不只是提出一个更好的 memory provider，而是把"如何记忆"从人工固定设计变成可进化对象。这个方向是合理的，但要继续往前走，需要把写入验证、检索相关性、记忆健康度、注入时机和统计稳健性都纳入进化空间。换句话说，memory architecture 应该随任务分布而变；而当任务瓶颈已经越过 memory 层，完整的 harness self-evolution 才是更高上限的方向。
